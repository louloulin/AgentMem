...
impl EmbeddedVectorStore {
        ...
        #[cfg(feature = "embedded-vector")]
        {
            ...
            let path_str = if self.config.in_memory {
                ...
                self.config
                    .path
                    .to_str()
                    .ok_or_else(|| AgentMemError::internal_error("无效的路径"))?
            };
    ...
    pub async fn get_store(&self) -> Result<Arc<LanceDBVectorStore>> {
        ...
        let store = store_lock
            .as_ref()
            .ok_or_else(|| AgentMemError::internal_error("向量存储未初始化"))?;
    ...
    pub async fn backup<P: AsRef<Path>>(&self, dest: P) -> Result<()> {
        ...
        // 创建目标目录
        tokio::fs::create_dir_all(dest.as_ref())
            .await
...
fn copy_dir_all(src: &Path, dst: &Path) -> Result<()> {
    ...
    for entry in std::fs::read_dir(src)
        .map_err(|e| AgentMemError::internal_error(format!("读取目录失败: {e}")))?
    {
        let entry =
            entry.map_err(|e| AgentMemError::internal_error(format!("读取目录项失败: {e}")))?;

        let ty = entry
            .file_type()
            .map_err(|e| AgentMemError::internal_error(format!("获取文件类型失败: {e}")))?;
