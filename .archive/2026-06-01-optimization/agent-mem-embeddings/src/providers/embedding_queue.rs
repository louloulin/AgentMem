...
impl EmbeddingQueue {
    ...
    /// 处理一批请求
    async fn process_batch(embedder: &Arc<dyn Embedder + Send + Sync>, batch: Vec<EmbedRequest>) {
        if batch.is_empty() {
            return;
        }
        ...
        match embedder.embed_batch(&texts).await {
            Ok(embeddings) => {
                ...
                for (i, (req, embedding)) in
                    batch.into_iter().zip(embeddings.into_iter()).enumerate()
                {
            ...
            Err(e) => {
                ...
                for req in batch.into_iter() {
                    let _ = req
                        .responder
                        .send(Err(AgentMemError::embedding_error(error_msg.clone())));
                }
            }
        }
    }
...
mod tests {
    ...
    async fn test_embedding_queue() {
        ...
        for i in 0..20 {
            ...
            let task =
                tokio::spawn(async move { queue_clone.embed(format!("Test text {}", i)).await });
            tasks.push(task);
        }
