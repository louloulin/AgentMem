...
2. **关键差距**:
   - ❌ **缺少智能记忆提取和更新机制** (Mem0 的核心优势)
   - ❌ **缺少图数据库集成** (MIRIX 和 Mem0 都有)
   - ❌ **缺少记忆去重和合并** (Mem0 的 ADD/UPDATE/DELETE 智能决策)
   - ❌ **缺少多模态支持** (图片、文件等)
   - ❌ **缺少简化的 SDK 接口** (MIRIX 的 `add()`, `chat()` 简洁 API)

3. **MVP 优先级**:
   - **P0**: 智能记忆提取、去重合并、简化 SDK
   - **P1**: 图数据库集成、多模态支持
   - **P2**: 高级搜索、记忆摘要
...
- ❌ 无去重检查
- ❌ 无智能更新决策
- ❌ API 过于复杂 (需要手动指定 agent_id, memory_type 等)
