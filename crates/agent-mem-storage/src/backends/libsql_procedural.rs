...
fn row_to_item(row: &Row) -> Result<ProceduralMemoryItem> {
    let steps_json: String = row
        .get(6)
        .map_err(|e| AgentMemError::storage_error(format!("Failed to get steps: {e}")))?;
    ...
    let success_rate_f64: f64 = row
        .get(7)
        .map_err(|e| AgentMemError::storage_error(format!("Failed to get success_rate: {e}")))?;

    let execution_count_i64: i64 = row
        .get(8)
        .map_err(|e| AgentMemError::storage_error(format!("Failed to get execution_count: {e}")))?;
    ...
    Ok(ProceduralMemoryItem {
        ...
        skill_name: row
            .get(4)
            .map_err(|e| AgentMemError::storage_error(format!("Failed to get skill_name: {e}")))?,
        description: row
            .get(5)
            .map_err(|e| AgentMemError::storage_error(format!("Failed to get description: {e}")))?,
        steps,
...
impl ProceduralMemoryStore for LibSqlProceduralStore {
    ...
    async fn query_items(
        ...
        let mut rows = match params_vec.len() {
            ...
            2 => {
                stmt.query(params![params_vec[0].clone(), params_vec[1].clone()])
                    .await
            }
            3 => {
                stmt.query(params![
