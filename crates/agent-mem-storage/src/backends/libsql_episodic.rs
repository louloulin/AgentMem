...
impl EpisodicMemoryStore for LibSqlEpisodicStore {
    ...
    async fn get_recent_events(&self, user_id: &str, limit: i64) -> Result<Vec<EpisodicEvent>> {
        ...
        let mut stmt = conn
            .prepare(
                "SELECT * FROM episodic_events WHERE user_id = ? ORDER BY occurred_at DESC LIMIT ?",
            )
            .await
            .map_err(|e| AgentMemError::storage_error(format!("Failed to prepare query: {e}")))?;
...
fn row_to_event(row: &libsql::Row) -> Result<EpisodicEvent> {
    Ok(EpisodicEvent {
        ...
        occurred_at: {
            let s: String = row.get(4).map_err(|e| {
                AgentMemError::storage_error(format!("Failed to get occurred_at: {e}"))
            })?;
            DateTime::parse_from_rfc3339(&s)
                .map_err(|e| {
                    AgentMemError::storage_error(format!("Failed to parse occurred_at: {e}"))
                })?
                .with_timezone(&Utc)
        },
        event_type: row
            .get(5)
            .map_err(|e| AgentMemError::storage_error(format!("Failed to get event_type: {e}")))?,
        actor: row
            .get(6)
            .map_err(|e| AgentMemError::storage_error(format!("Failed to get actor: {e}")))?,
        summary: row
            .get(7)
        ...
        importance_score: {
            let score: f64 = row.get(9).map_err(|e| {
                AgentMemError::storage_error(format!("Failed to get importance_score: {e}"))
            })?;
            score as f32
        },
        metadata: {
        ...
        updated_at: {
            let s: String = row.get(12).map_err(|e| {
