...
impl QueryOptimizer {
    ...
    pub async fn explain_query(&self, query: &str) -> Result<QueryPlan, sqlx::Error> {
        ...
        let row = sqlx::query(&explain_query).fetch_one(&self.pool).await?;
    ...
    fn parse_execution_time(plan_json: &serde_json::Value) -> Option<Duration> {
        ...
        let execution_time_ms = plan.get("Actual Total Time").and_then(|v| v.as_f64())?;
    ...
    fn generate_warnings(plan_json: &serde_json::Value, query: &str) -> Vec<String> {
        ...
        if !query.to_uppercase().contains("WHERE") && !query.to_uppercase().contains("LIMIT") {
            warnings.push(
                "Query has no WHERE clause or LIMIT. This may scan the entire table.".to_string(),
            );
        }
        ...
        if query.to_uppercase().contains("SELECT *") {
            warnings
                .push("Query uses SELECT *. Consider selecting only needed columns.".to_string());
        }
        ...
        {
            warnings.push(
                "Query on large table without LIMIT. Consider adding pagination.".to_string(),
            );
        }
        ...
        if query.contains("IN (") {
            ...
            if in_clause_size > 1 {
                warnings.push(
                    "Multiple IN clauses detected. Consider using JOINs instead.".to_string(),
                );
            }
        }
