...
async fn test_user_crud_operations() {
    ...
    let repositories = RepositoryFactory::create_repositories(&config)
        .await
        .unwrap();
    ...
    let exists = user_repo
        .email_exists("test@example.com", &org.id)
        .await
        .unwrap();
    assert!(exists);

    let not_exists = user_repo
        .email_exists("nonexistent@example.com", &org.id)
        .await
        .unwrap();
    assert!(!not_exists);
    ...
    let result = user_repo
        .update_password(&created_user.id, "new_hashed_password")
        .await;
    assert!(result.is_ok());
...
async fn test_agent_crud_operations() {
    ...
    // Find by ID
    let found_agent = repositories
        .agents
        .find_by_id(&created_agent.id)
        .await
        .unwrap();
    assert!(found_agent.is_some());

    // Find by organization
    let org_agents = repositories
        .agents
...
async fn test_message_operations() {
    ...
    // Find by user
    let user_messages = repositories
        .messages
        .find_by_user_id(&user.id, 10)
        .await
        .unwrap();
...
async fn test_tool_operations() {
    ...
    // Find by tags
    let tools_by_tag = repositories
        .tools
        .find_by_tags(&org.id, &["testing".to_string()])
        .await
        .unwrap();
    assert_eq!(tools_by_tag.len(), 1);

    // Find by organization
    let org_tools = repositories
        .tools
        .find_by_organization_id(&org.id)
        .await
        .unwrap();
...
async fn test_concurrent_operations() {
    ...
    let repositories = Arc::new(
        RepositoryFactory::create_repositories(&config)
            .await
            .unwrap(),
    );
