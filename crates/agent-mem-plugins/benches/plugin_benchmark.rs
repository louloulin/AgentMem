...
async fn benchmark_plugin_execution() {
    ...
    let _ = manager.call_plugin("bench-exec", "hello", &input.to_string()).await;
    ...
    for i in 0..iterations {
        ...
        let _ = manager.call_plugin("bench-exec", "hello", &input.to_string()).await.unwrap();
    }
...
async fn benchmark_cache_performance() {
    ...
    for cache_size in cache_sizes {
        ...
        for i in 0..iterations {
            ...
            let _ = manager.call_plugin(&plugin_id, "hello", &input.to_string()).await;
        }
    ...
    for concurrency in concurrency_levels {
        ...
        for i in 0..concurrency {
            ...
            let handle = tokio::spawn(async move {
                ...
                mgr.call_plugin("concurrent-test", "hello", &input.to_string()).await
            });
            handles.push(handle);
        }
        ...
        println!("  Concurrency {:<3}:  {:?} total, {:?} avg", 
...
async fn benchmark_memory_processing() {
    ...
    for size in sizes {
        ...
        for _ in 0..iterations {
            let _ = manager.call_plugin("memory-bench", "process_memory", &input).await.unwrap();
        }
        
        let total_time = start.elapsed();
        let avg_time = total_time / iterations as u32;
        let throughput = (content.len() * iterations) as f64 / total_time.as_secs_f64() / 1024.0 / 1024.0;
        
        println!("  Content size {:<4} chars:  {:?}/call, {:.2} MB/s", 
                 content.len(), avg_time, throughput);
    }
}
