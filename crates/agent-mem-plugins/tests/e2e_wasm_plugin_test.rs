...
use agent_mem_plugins::{PluginLoader, PluginManager, PluginStatus, RegisteredPlugin};
use chrono::Utc;
...
async fn test_multiple_wasm_plugins_concurrent() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let plugins = vec![
        (
            "hello_plugin.wasm",
            "hello-1",
            PluginType::Custom("hello".to_string()),
        ),
        (
            "memory_processor_plugin.wasm",
            "processor-1",
            PluginType::MemoryProcessor,
        ),
        (
            "code_analyzer_plugin.wasm",
            "analyzer-1",
