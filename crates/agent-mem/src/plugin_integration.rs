...
#[cfg(feature = "plugins")]
use agent_mem_plugins::sdk::{Capability, PluginConfig, PluginMetadata, PluginType};
#[cfg(feature = "plugins")]
use agent_mem_plugins::{PluginManager, PluginRegistry, PluginStatus, RegisteredPlugin};
    ...
    pub async fn process_memory_with_plugins(&self, memory: &mut MemoryItem) -> Result<()> {
        ...
        for plugin_info in plugins {
            if matches!(
                ...
                match self
                    .manager
                    .call_plugin(&plugin_info.id, "process_memory", &input)
                    .await
                {
                    Ok(output) => {
                        ...
                        match serde_json::from_str::<MemoryItem>(&output) {
                            Ok(processed_memory) => {
                                *memory = processed_memory;
                                tracing::debug!(
                                    "Memory processed successfully by plugin: {}",
                                    plugin_info.metadata.name
                                );
                            }
                            Err(e) => {
                                tracing::warn!(
                                    "Plugin {} returned invalid memory format: {}",
                                    plugin_info.metadata.name,
                                    e
                                );
                            }
    ...
    pub async fn search_with_plugin(
        ...
        for plugin_info in plugins {
            if matches!(
                ...
                tracing::debug!("Searching with plugin: {}", plugin_info.metadata.name);
                ...
                match self
                    .manager
                    .call_plugin(&plugin_info.id, "search", &input)
                    .await
                {
                    Ok(output) => {
                        ...
                        match serde_json::from_str::<Vec<MemoryItem>>(&output) {
                            Ok(results) => {
                                tracing::debug!(
                                    "Search completed by plugin: {}, found {} results",
                                    plugin_info.metadata.name,
                                    results.len()
                                );
                                return Ok(results);
                            ...
                            Err(e) => {
                                tracing::warn!(
                                    "Plugin {} returned invalid search results: {}",
                                    plugin_info.metadata.name,
                                    e
                                );
                            }
                    ...
                    Ok(output) => {
                        ...
                        match serde_json::from_str::<Vec<MemoryItem>>(&output) {
                            Ok(reranked) => {
                                ...
                                tracing::debug!(
                                    "Results reranked by plugin: {}",
                                    plugin_info.metadata.name
                                );
                            }
                            Err(e) => {
                                tracing::warn!(
                                    "Plugin {} returned invalid rerank format: {}",
                                    plugin_info.metadata.name,
                                    e
                                );
                            }
