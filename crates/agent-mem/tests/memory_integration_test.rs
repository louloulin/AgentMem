...
async fn test_add_memory() {
    ...
    assert!(result.is_ok(), "Should add memory successfully: {:?}", result.err());
...
async fn test_search_memory() {
    ...
    // Add some memories
    memory.add("I love pizza").await.expect("Failed to add memory 1");
    memory.add("Python is great").await.expect("Failed to add memory 2");
    memory.add("Rust provides memory safety").await.expect("Failed to add memory 3");
    ...
    let has_pizza = results.iter().any(|r| r.content.to_lowercase().contains("pizza"));
...
async fn test_get_all_memories() {
    ...
    memory.add("Memory 1").await.expect("Failed to add memory 1");
    memory.add("Memory 2").await.expect("Failed to add memory 2");
    memory.add("Memory 3").await.expect("Failed to add memory 3");
    
    // Get all
    let all_memories = memory.get_all(agent_mem::GetAllOptions::default()).await;
    
    assert!(all_memories.is_ok(), "Get all should succeed: {:?}", all_memories.err());
    let all_memories = all_memories.unwrap();
    assert!(all_memories.len() >= 3, "Should have at least 3 memories, got {}", all_memories.len());
}
...
async fn test_delete_memory() {
    ...
    // Add a memory
    let add_result = memory.add("To be deleted").await.expect("Failed to add memory");
...
async fn test_delete_all_memories() {
    ...
    assert!(count.is_ok(), "Delete all should succeed: {:?}", count.err());
...
async fn test_memory_workflow() {
    ...
    // 1. Add memories
    let r1 = memory.add("I like programming in Rust").await.expect("Failed to add r1");
    let r2 = memory.add("Python is also great").await.expect("Failed to add r2");
    let r3 = memory.add("Rust provides memory safety").await.expect("Failed to add r3");
    ...
    // 3. Get all memories
    let all_memories = memory.get_all(agent_mem::GetAllOptions::default()).await.expect("Failed to get all");
    ...
    let remaining = memory.get_all(agent_mem::GetAllOptions::default()).await.expect("Failed to get remaining");
    ...
    assert!(ids.contains(&id1.as_str()), "Non-deleted memory should still exist");
}
...
async fn test_chinese_content() {
    ...
    assert!(result.is_ok(), "Should handle Chinese content: {:?}", result.err());
    let add_result = result.unwrap();
...
async fn test_empty_search() {
    ...
    memory.add("Test content").await.expect("Failed to add test content");
    
    // Empty search
    let results = memory.search("").await;
    
    // Empty search should return empty results
    assert!(results.is_ok(), "Empty search should not crash: {:?}", results.err());
}
...
async fn test_memory_clone() {
    ...
    // Add a memory
    memory1.add("Test memory").await.expect("Failed to add test memory");
    ...
    assert!(results1.is_ok(), "Memory1 search should work: {:?}", results1.err());
    assert!(results2.is_ok(), "Memory2 search should work: {:?}", results2.err());
}
...
async fn test_concurrent_operations() {
    ...
    println!("Concurrent test: {} successful, {} failed", success_count, error_count);
...
async fn test_special_characters() {
    ...
    for case in test_cases {
        ...
        assert!(result.is_ok(), "Should handle special chars in: {} - {:?}", case, result.err());
    }
}
...
async fn test_multiple_searches() {
    ...
    // Add diverse content
    memory.add("Rust programming language").await.expect("Failed to add");
    memory.add("Python data science").await.expect("Failed to add");
    memory.add("JavaScript web development").await.expect("Failed to add");
...
async fn test_builder_pattern() {
    ...
    assert!(memory.is_ok(), "Builder pattern should work: {:?}", memory.err());
}
...
async fn test_multiple_instances() {
    ...
    // Add to both
    mem1.add("Memory in instance 1").await.expect("Failed to add to mem1");
    mem2.add("Memory in instance 2").await.expect("Failed to add to mem2");
    
    // Both should have their own data
    let count1 = mem1.get_all(agent_mem::GetAllOptions::default()).await.expect("Failed to get all from mem1");
    let count2 = mem2.get_all(agent_mem::GetAllOptions::default()).await.expect("Failed to get all from mem2");
