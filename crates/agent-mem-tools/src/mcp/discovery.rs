...
use super::types::{McpTool, McpListToolsResponse};
use super::transport::{Transport, HttpTransport};
...
impl ToolDiscovery {
    ...
    pub async fn register_metadata(&self, metadata: ToolMetadata) -> McpResult<()> {
        info!("Registering tool metadata: {} v{}", metadata.name, metadata.version);
...
impl ToolLoader for HttpToolLoader {
    async fn load(&self, metadata: &ToolMetadata) -> McpResult<bool> {
        ...
        info!("Loading HTTP tool: {} from {}", metadata.name, metadata.source);
