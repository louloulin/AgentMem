...
use agent_mem_core::search::{
    IndexStatistics, OptimizedSearchPlan, QueryOptimizer, QueryOptimizerConfig,
    RerankConfig, ResultReranker, SearchQuery, SearchResult, SearchStrategy,
};
...
async fn test_result_reranker_basic() {
    ...
    let candidates = vec![
        ...
        create_test_result("2", 0.9, "This is a test content with good length for quality scoring"),
...
async fn test_performance_baseline() {
    ...
    let _reranked = reranker.rerank(candidates, &query_vector, &query).await.unwrap();
