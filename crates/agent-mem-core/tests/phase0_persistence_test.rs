...
async fn test_phase0_batch_persistence() {
    ...
    // 批量写入
    let ids = operations
        .batch_create_memories(memories.clone())
        .await
    ...
    // 验证读取
    let agent_memories = operations
        .get_agent_memories("batch_agent", Some(10))
        .await
