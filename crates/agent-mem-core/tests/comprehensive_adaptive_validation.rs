...
fn test_extreme_length_queries() {
    ...
    for query_text in very_short {
        ...
        let (_, weights) = optimizer.optimize_query(&query);
        println!("查询: \"{}\" → vector={:.3}, fulltext={:.3}", 
    ...
    // 极长查询
    let very_long = "This is an extremely long query that contains multiple sentences and paragraphs. ".repeat(20);
...
fn test_performance_degradation() {
    ...
    // 平均每个查询应该在100μs以内
    assert!(avg_time < 100.0, "性能退化：平均查询时间 {:.2} μs 超过100 μs", avg_time);
...
fn test_feature_extraction_accuracy() {
    ...
    for query in temporal_queries {
        let features = QueryFeatures::extract_from_query(query);
        assert!(features.has_temporal_indicator, "应该检测到时间指示: {}", query);
    }
    println!("✅ 时间指示词检测准确");
    
    // 测试3: 问句检测
    let question_queries = vec!["What is AI?", "How does it work?", "Why?", "什么是AI？", "怎么做？"];
    ...
    assert!(simple.semantic_complexity < complex.semantic_complexity,
        "复杂查询的语义复杂度应该更高");
    println!("✅ 语义复杂度计算合理");
}
...
fn test_weight_prediction_rationality() {
    ...
    assert!(weights.fulltext_weight > 0.5, 
        "精确匹配查询应该偏向全文搜索，实际: {:.2}", weights.fulltext_weight);
    println!("✅ 精确匹配权重合理");
    ...
    assert!(weights.vector_weight > 0.6,
        "高语义复杂度应该偏向向量搜索，实际: {:.2}", weights.vector_weight);
    println!("✅ 语义查询权重合理");
