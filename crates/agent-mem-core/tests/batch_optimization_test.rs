...
use agent_mem_core::embeddings_batch::{
    EmbeddingBatchConfig, EmbeddingBatchProcessor, EmbeddingBatchStats,
...
async fn test_embedding_batch_empty_texts() {
    ...
    let embed_fn = |batch: Vec<String>| async move {
        Ok(vec![vec![0.1, 0.2, 0.3]; batch.len()])
    };
...
async fn test_embedding_batch_stats_reset() {
    ...
    let embed_fn = |batch: Vec<String>| async move {
        Ok(vec![vec![1.0]; batch.len()])
    };
    
    // First batch
    let _ = processor.batch_embed(texts.clone(), |b| embed_fn(b)).await.unwrap();
...
async fn test_embedding_batch_different_sizes() {
    ...
    for size in [1, 5, 10, 15, 20, 50, 100] {
        ...
        let result = processor.batch_embed(texts.clone(), embed_fn).await.unwrap();
