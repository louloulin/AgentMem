    ...
    async fn execute(
        ...
        if self.extract_persons {
            if let Ok(name_pattern) =
                regex::Regex::new(r"(张|李|王|刘|陈|杨|黄|赵|吴|周)[\p{Han}]{1,3}")
            {
        ...
        if self.extract_money {
            ...
            if let Ok(money_pattern) = regex::Regex::new(
                r"(?:[$¥€£]\s*\d+(?:\.\d{2})?|\d+(?:\.\d{2})?\s*(?:元|美元|欧元|英镑|人民币|USD|CNY|EUR|GBP))",
            ) {
        ...
        if self.extract_time {
            ...
            if let Ok(time_pattern) =
                regex::Regex::new(r"(?:\d{1,2}:\d{2}(?::\d{2})?|(?:上午|下午|早上|晚上)\d{1,2}点)")
            {
        ...
        if self.extract_percentage {
            ...
            if let Ok(percent_pattern) =
                regex::Regex::new(r"(?:\d+(?:\.\d+)?%|百分之\d+(?:\.\d+)?)")
            {
...
impl PipelineStage for QueryExpansionStage {
    ...
    async fn execute(
        ...
        if !expanded_terms.is_empty() {
            ...
            // 记录扩展的同义词（供后续阶段使用）
            let synonym_list: Vec<String> = expanded_terms
                .iter()
...
impl PipelineStage for RelationBuildingStage {
    ...
    async fn execute(
        ...
        for existing in existing_memories.iter().take(100) {
            // Limit to 100 recent memories
            ...
            if self.enable_entity && !current_entities.is_empty() {
                if let Some(AttributeValue::Array(existing_entities)) =
                    existing.attributes.get(&AttributeKey::domain("entities"))
                {
                    let existing_entity_strs: Vec<String> = existing_entities
                        .iter()
                        .filter_map(|e| {
                    ...
                    let shared_count = current_entities
                        .iter()
            ...
            if self.enable_temporal {
                let time_diff = (current_memory.metadata.created_at.timestamp()
                    - existing.metadata.created_at.timestamp())
                .abs();

                if time_diff < self.temporal_window_secs {
                    let temporal_score =
                        1.0 - (time_diff as f32 / self.temporal_window_secs as f32);
                    relation_strength += temporal_score * 0.3;
                    relation_reasons.push(format!(
                        "temporal_proximity:{:.2}h",
                        time_diff as f32 / 3600.0
                    ));
                }
            }
            ...
            if relation_strength >= 0.3 {
                current_memory
                    .relations
                    .add_relation(crate::types::Relation {
                        target_id: existing.id.clone(),
                        relation_type: crate::types::RelationType::Custom(format!(
                            "auto_discovered: {}",
                            relation_reasons.join(", ")
                        )),
...
impl PipelineStage for MemoryCompressionStage {
    ...
    async fn execute(
        ...
        for i in 0..input.len() {
            ...
            if group.len() > 1 {
                let memories_to_merge: Vec<_> = group.iter().map(|&idx| &input[idx]).collect();
...
impl MemoryCompressionStage {
    ...
    fn calculate_attribute_similarity(&self, mem1: &Memory, mem2: &Memory) -> f32 {
        ...
        let agent1 = mem1
            .attributes
            .get(&crate::types::AttributeKey::system("agent_id"))
        ...
        let user1 = mem1
            .attributes
            .get(&crate::types::AttributeKey::system("user_id"))
            .and_then(|v| v.as_string());
        let user2 = mem2
            .attributes
            .get(&crate::types::AttributeKey::system("user_id"))
        ...
        let type1 = mem1
            .attributes
            .get(&crate::types::AttributeKey::system("memory_type"))
            .and_then(|v| v.as_string());
        let type2 = mem2
            .attributes
            .get(&crate::types::AttributeKey::system("memory_type"))
    ...
    fn merge_memories(&self, memories: &[&Memory]) -> Memory {
        ...
        if memories.len() > 1 {
            let all_texts: Vec<String> = memories.iter().map(|m| m.content.to_string()).collect();
        ...
        if self.preserve_unique_entities {
            ...
            for memory in memories {
                if let Some(entities) = memory
                    .attributes
                    .get(&crate::types::AttributeKey::system("entities"))
                {
            ...
            match constraint {
                ...
                Constraint::MinScore(score) => {
                    if *score < 0.0 || *score > 1.0 {
                        return Ok(StageResult::Abort(
                            "MinScore must be between 0 and 1".to_string(),
                        ));
                    }
                }
                _ => {}
...
mod tests {
    ...
    async fn test_entity_extraction_enhanced() {
        ...
        if let StageResult::Continue(_mem) = result {
            ...
            assert!(
                entities.iter().any(|e| e.starts_with("DATE:")),
                "Should have DATE entity"
            );
            assert!(
                entities.iter().any(|e| e.starts_with("MONEY:")),
                "Should have MONEY entity"
            );
            assert!(
                entities.iter().any(|e| e.starts_with("PERCENTAGE:")),
                "Should have PERCENTAGE entity"
            );
            assert!(
                entities.iter().any(|e| e.starts_with("IP:")),
                "Should have IP entity"
            );
            assert!(
                entities.iter().any(|e| e.starts_with("TIME:")),
    ...
    async fn test_query_understanding_stage() {
        ...
        assert!(matches!(result, StageResult::Continue(_)));
        assert_eq!(
            context.get::<String>("intent_type"),
    ...
    async fn test_relation_building_stage() {
        ...
        let result = stage
            .execute((current_memory, vec![existing1, existing2]), &mut context)
            .await
            .unwrap();
        ...
        if let StageResult::Continue(memory) = result {
            ...
            assert!(
                relations_built > 0,
                "Should have built at least one relation"
            );

            // Check that relation was established
            assert!(
                !memory.relations.relations().is_empty(),
                "Should have established relations"
            );
        } else {
            panic!("Expected Continue");
        }
    ...
    async fn test_memory_compression_stage() {
        ...
        if let StageResult::Continue(compressed) = result {
            ...
            if compressed.len() < original_count {
                assert!(
                    ratio < 1.0,
                    "Compression ratio should be < 1.0 when compressed"
                );

                // Check merged memory has boost
                let merged_memory = compressed.iter().find(|m| {
                    m.attributes
                        .get(&crate::types::AttributeKey::system("merged_from_count"))
                        .is_some()
                });

                assert!(
                    merged_memory.is_some(),
                    "Should have at least one merged memory"
                );
            } else {
    ...
    async fn test_importance_reassessment_stage() {
        ...
        if let StageResult::Continue(updated_memory) = result {
            ...
            assert!(
                factors.contains("access_freq"),
                "Should have access frequency factor"
            );
            assert!(
                factors.contains("relation_boost"),
                "Should have relation boost factor"
            );
        } else {
            panic!("Expected Continue");
        }
    ...
    async fn test_query_expansion_stage() {
        ...
        match result {
            StageResult::Continue(_) => {
                ...
                let has_relation = context
                    .get::<bool>("relation_expansion_enabled")
                    .unwrap_or(false);
                ...
                // 如果找到了扩展项，验证它们
                if let Some(expanded_terms) =
                    context.get::<Vec<(String, Vec<String>)>>("expanded_terms")
                {
                    println!("Found expanded terms: {:?}", expanded_terms);
                    assert!(has_expanded, "应该标记为已扩展");
                }

                if let Some(expanded_relations) =
                    context.get::<Vec<(String, String)>>("expanded_relations")
                {
