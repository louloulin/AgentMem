...
impl MessageRepositoryTrait for LibSqlMessageRepository {
    ...
    async fn find_by_id(&self, id: &str) -> Result<Option<Message>> {
            ...
            .map_err(|e| AgentMemError::StorageError(format!("Failed to fetch row: {e}")))?
        {
            let message = Message {
                ...
                user_id: row.get::<String>(2).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get user_id: {e}"))
                })?,
                agent_id: row.get::<String>(3).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get agent_id: {e}"))
                })?,
                role: row
                    .get::<String>(4)
                    .map_err(|e| AgentMemError::StorageError(format!("Failed to get role: {e}")))?,
                text: row
                    .get::<Option<String>>(5)
                    .map_err(|e| AgentMemError::StorageError(format!("Failed to get text: {e}")))?,
                content: Self::deserialize_json(row.get::<Option<String>>(6).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get content: {e}"))
                })?),
                model: row.get::<Option<String>>(7).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get model: {e}"))
                })?,
                name: row
                    .get::<Option<String>>(8)
                    .map_err(|e| AgentMemError::StorageError(format!("Failed to get name: {e}")))?,
                tool_calls: Self::deserialize_json(row.get::<Option<String>>(9).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get tool_calls: {e}"))
                })?),
                tool_call_id: row.get::<Option<String>>(10).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get tool_call_id: {e}"))
                })?,
                step_id: row.get::<Option<String>>(11).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get step_id: {e}"))
                })?,
                otid: row
                    .get::<Option<String>>(12)
                    .map_err(|e| AgentMemError::StorageError(format!("Failed to get otid: {e}")))?,
                tool_returns: Self::deserialize_json(row.get::<Option<String>>(13).map_err(
                    |e| AgentMemError::StorageError(format!("Failed to get tool_returns: {e}")),
                )?),
                group_id: row.get::<Option<String>>(14).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get group_id: {e}"))
                })?,
                sender_id: row.get::<Option<String>>(15).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get sender_id: {e}"))
                })?,
    ...
    async fn delete_by_agent_id(&self, agent_id: &str) -> Result<u64> {
        ...
        let rows_affected = conn
            ...
            .await
            .map_err(|e| {
                AgentMemError::StorageError(format!("Failed to delete messages by agent_id: {e}"))
            })?;
