    ...
    async fn find_by_id(&self, id: &str) -> Result<Option<Agent>> {
            ...
            .map_err(|e| AgentMemError::StorageError(format!("Failed to fetch row: {e}")))?
        {
            let agent = Agent {
                ...
                organization_id: row.get::<String>(1).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get organization_id: {e}"))
                })?,
                agent_type: row.get::<Option<String>>(2).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get agent_type: {e}"))
                })?,
                name: row
                    .get::<Option<String>>(3)
                    .map_err(|e| AgentMemError::StorageError(format!("Failed to get name: {e}")))?,
                description: row.get::<Option<String>>(4).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get description: {e}"))
                })?,
                system: row.get::<Option<String>>(5).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get system: {e}"))
                })?,
                topic: row.get::<Option<String>>(6).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get topic: {e}"))
                })?,
                message_ids: Self::deserialize_vec(row.get::<Option<String>>(7).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get message_ids: {e}"))
                })?),
                metadata_: Self::deserialize_json(row.get::<Option<String>>(8).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get metadata_: {e}"))
                })?),
                llm_config: Self::deserialize_json(row.get::<Option<String>>(9).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get llm_config: {e}"))
                })?),
                embedding_config: Self::deserialize_json(row.get::<Option<String>>(10).map_err(
                    |e| AgentMemError::StorageError(format!("Failed to get embedding_config: {e}")),
                )?),
                tool_rules: Self::deserialize_json(row.get::<Option<String>>(11).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get tool_rules: {e}"))
                })?),
                mcp_tools: Self::deserialize_vec(row.get::<Option<String>>(12).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get mcp_tools: {e}"))
                })?),
                state: row.get::<Option<String>>(13).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get state: {e}"))
                })?,
                last_active_at: row
                    .get::<Option<i64>>(14)
                    .map_err(|e| {
                        AgentMemError::StorageError(format!("Failed to get last_active_at: {e}"))
                    })?
                    .and_then(|ts| chrono::DateTime::from_timestamp(ts, 0)),
                error_message: row.get::<Option<String>>(15).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get error_message: {e}"))
                })?,
                created_at: chrono::DateTime::from_timestamp(
                    row.get::<i64>(16).map_err(|e| {
                ...
                is_deleted: row.get::<i64>(18).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get is_deleted: {e}"))
                })? != 0,
                created_by_id: row.get::<Option<String>>(19).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get created_by_id: {e}"))
                })?,
                last_updated_by_id: row.get::<Option<String>>(20).map_err(|e| {
                    AgentMemError::StorageError(format!("Failed to get last_updated_by_id: {e}"))
                })?,
            };
            Ok(Some(agent))
        } else {
