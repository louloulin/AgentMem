    ...
    async fn update_statistics(&self, pattern: QueryPattern, record: FeedbackRecord) {
        ...
        if pattern_stat.total_queries == 0 {
            ...
            if record.effectiveness > pattern_stat.avg_effectiveness * 1.1 {
                // 向更好的权重移动
                pattern_stat.best_weights.vector_weight = alpha * record.weights.vector_weight
                    + (1.0 - alpha) * pattern_stat.best_weights.vector_weight;
                pattern_stat.best_weights.fulltext_weight = alpha * record.weights.fulltext_weight
...
mod tests {
    ...
    async fn test_learning_engine_basic() {
        ...
        for i in 0..15 {
            ...
            engine
                .record_feedback(features.clone(), weights.clone(), effectiveness, None)
                .await;
        }
