...
impl QueryFeatures {
    ...
    pub fn extract_from_query(query: &str) -> Self {
        ...
        // 检测时间指示词
        let temporal_keywords = ["yesterday", "today", "tomorrow", "last week", 
                                 "last month", "recently", "昨天", "今天", "最近"];
        let has_temporal_indicator = temporal_keywords.iter()
        ...
        // 简单的实体计数（大写词、@提及等）
        let entity_count = query.split_whitespace()
...
impl AdaptiveSearchOptimizer {
    ...
    pub fn record_feedback(
        ...
        if self.enable_learning {
            self.weight_predictor.learn_from_feedback(query, weights, effectiveness);
        }
    }
}
