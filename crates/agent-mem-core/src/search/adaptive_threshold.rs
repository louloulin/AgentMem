...
impl AdaptiveThresholdCalculator {
    ...
    pub async fn calculate_with_details(
        ...
        let length_adjustment =
            self.calculate_length_adjustment(features.length) * self.config.length_factor;
        ...
        let mut threshold =
            base_threshold + length_adjustment + complexity_adjustment + historical_adjustment;
...
mod tests {
    ...
    async fn test_adaptive_threshold_exact_id() {
        ...
        let threshold = calculator
            .calculate(query, &QueryType::ExactId, &features)
            .await;
    ...
    async fn test_adaptive_threshold_short_keyword() {
        ...
        let features = classifier.extract_features(query);
        let threshold = calculator
            .calculate(query, &QueryType::ShortKeyword, &features)
            .await;
    ...
    async fn test_adaptive_threshold_semantic() {
        ...
        let features = classifier.extract_features(query);
        let threshold = calculator
            .calculate(query, &QueryType::Semantic, &features)
            .await;
    ...
    async fn test_complexity_calculation() {
        ...
        let complex_complexity = calculator
            .calculate_complexity("What's the best way to learn Rust?", &complex_features);
