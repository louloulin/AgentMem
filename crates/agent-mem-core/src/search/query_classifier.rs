...
impl QueryClassifier {
    ...
    pub fn extract_features(&self, query: &str) -> QueryFeatures {
        ...
        let has_special_chars = trimmed.chars().any(|c| {
        ...
        let is_question = trimmed.contains('?') || 
            ...
            self.question_words.iter().any(|word| {
                trimmed.to_lowercase().starts_with(word)
            });
        
        let has_numbers = trimmed.chars().any(|c| c.is_numeric());
        let is_uppercase = trimmed.chars().filter(|c| c.is_alphabetic()).all(|c| c.is_uppercase());
    ...
    fn detect_language(&self, query: &str) -> Language {
        ...
        let has_english = query.chars().any(|c| {
    ...
    pub fn get_adaptive_strategy(&self, query: &str) -> (QueryType, SearchStrategy) {
        ...
        // 如果包含很多数字，增加BM25权重
        let digit_ratio = query.chars().filter(|c| c.is_numeric()).count() as f32 
            / query.len() as f32;
        if digit_ratio > 0.3 {
...
mod tests {
    ...
    fn test_adaptive_strategy() {
        ...
        let (query_type, _) = classifier.get_adaptive_strategy(
