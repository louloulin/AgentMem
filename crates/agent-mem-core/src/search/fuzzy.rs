...
#[async_trait]
impl SearchEngine for FuzzyMatchEngine {
    /// 执行搜索查询（V4 Query 接口）
        ...
        let query_text = match &query.intent {
            ...
            _ => {
                return Err(agent_mem_traits::AgentMemError::validation_error(
                    format!("Unsupported query intent for FuzzyMatchEngine: {:?}. Use NaturalLanguage or Hybrid intent.", query.intent)
                ));
            }
        };
