...
mod tests {
    ...
    async fn test_fuzzy_match_basic() {
        ...
        engine.add_document("doc2".to_string(), "the quik brwn fox".to_string()).await.unwrap(); // 拼写错误
        engine.add_document("doc3".to_string(), "lazy dog".to_string()).await.unwrap();
    ...
    async fn test_fuzzy_match_case_insensitive() {
        ...
        engine.add_document("doc1".to_string(), "Hello World".to_string()).await.unwrap();
