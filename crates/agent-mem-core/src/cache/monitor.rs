...
impl CacheMonitor {
        ...
        if self.config.enable_alerts {
            ...
            if hit_rate < self.config.hit_rate_alert_threshold {
                warn!(
                    ...
                    hit_rate,
                    self.config.hit_rate_alert_threshold
                );
            }
        }
    ...
    pub async fn generate_report(&self) -> Option<PerformanceReport> {
        ...
        // 计算趋势
        let hit_rate_trend = latest.combined_stats.hit_rate() 
            - earliest.combined_stats.hit_rate();
        
        let avg_response_time_trend = latest.avg_response_time_ms 
            - earliest.avg_response_time_ms;
        
        // 计算平均值
        let avg_hit_rate = snapshots.iter()
            .map(|s| s.combined_stats.hit_rate())
            .sum::<f64>() / snapshots.len() as f64;
        
        let avg_qps = snapshots.iter()
        ...
        sorted_by_hit_rate.sort_by(|a, b| {
            b.combined_stats.hit_rate()
                .partial_cmp(&a.combined_stats.hit_rate())
                .unwrap()
        });
        
        let best_hit_rate = sorted_by_hit_rate.first().unwrap().combined_stats.hit_rate();
...
impl PerformanceReport {
    ...
    pub fn format_text(&self) -> String {
        ...
        output.push_str(&format!("报告周期: {} 秒 ({} 分钟)\n", 
            self.report_period_secs, self.report_period_secs / 60));
        output.push_str(&format!("快照数量: {}\n\n", self.total_snapshots));
        
        output.push_str("--- 命中率指标 ---\n");
        output.push_str(&format!("当前命中率: {:.2}%\n", 
