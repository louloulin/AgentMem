...
use super::{
    Cache, CacheKey, CacheMonitor, CacheStats, MemoryCache, MemoryCacheConfig, MonitorConfig,
};
...
impl MultiLevelCache {
    ...
    pub fn new(config: MultiLevelCacheConfig) -> Self {
        info!(
            "Creating multi-level cache (L1: {}, L2: {}, Monitoring: {})",
        ...
        if self.config.enable_l1 {
            if let Some(value) = self.l1.get(key).await? {
                ...
                if let Some(monitor) = &self.monitor {
                    monitor
                        .record_operation(start.elapsed(), hit, cache_level)
                        .await;
                }
    ...
    pub async fn performance_snapshot(&self) -> Result<Option<super::PerformanceSnapshot>> {
        if let Some(monitor) = &self.monitor {
            ...
            let snapshot = monitor
                .create_snapshot(l1_stats, l2_stats, combined_stats)
                .await;
            Ok(Some(snapshot))
        } else {
            Ok(None)
