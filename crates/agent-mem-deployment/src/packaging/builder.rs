...
impl PackageBuilder {
    ...
    async fn clean_output_dir(&self) -> Result<()> {
        ...
        if self.config.output_dir.exists() {
            tokio::fs::remove_dir_all(&self.config.output_dir)
                .await
                .map_err(|e| AgentMemError::internal_error(format!("清理输出目录失败: {e}")))?;
        }

        tokio::fs::create_dir_all(&self.config.output_dir)
            .await
            .map_err(|e| AgentMemError::internal_error(format!("创建输出目录失败: {e}")))?;
    ...
    async fn build_binary(&self) -> Result<PathBuf> {
        ...
        let output = cmd
            .output()
            .map_err(|e| AgentMemError::internal_error(format!("执行 cargo build 失败: {e}")))?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(AgentMemError::internal_error(format!("构建失败: {stderr}")));
        }
    ...
    async fn copy_to_output(&self, binary_path: &Path) -> Result<PathBuf> {
        ...
        tokio::fs::copy(binary_path, &output_path)
            .await
            .map_err(|e| AgentMemError::internal_error(format!("复制二进制文件失败: {e}")))?;
    ...
    async fn include_config_files(&self) -> Result<()> {
        ...
        tokio::fs::create_dir_all(&config_dir)
            .await
            .map_err(|e| AgentMemError::internal_error(format!("创建配置目录失败: {e}")))?;
    ...
    async fn include_docs(&self) -> Result<()> {
        ...
        tokio::fs::create_dir_all(&docs_dir)
            .await
            .map_err(|e| AgentMemError::internal_error(format!("创建文档目录失败: {e}")))?;
        ...
        if readme_src.exists() {
            ...
            tokio::fs::copy(&readme_src, &readme_dst)
                .await
                .map_err(|e| AgentMemError::internal_error(format!("复制 README 失败: {e}")))?;
        }
    ...
    async fn generate_metadata(&self, binary_path: &Path) -> Result<()> {
        ...
        let content = serde_json::to_string_pretty(&metadata)
            .map_err(|e| AgentMemError::internal_error(format!("序列化元数据失败: {e}")))?;

        tokio::fs::write(&metadata_path, content)
            .await
            .map_err(|e| AgentMemError::internal_error(format!("写入元数据失败: {e}")))?;
