...
use agent_mem_server::rbac::{Role, RbacChecker, Resource, Action, Permission};
...
async fn test_rbac_admin_permissions() {
    ...
    // 验证Admin角色有所有权限
    assert!(RbacChecker::check_resource_action(
        &admin_roles,
        Resource::Memory,
        Action::Read
    ).is_ok());
    ...
    assert!(RbacChecker::check_permission(
        &admin_roles,
...
async fn test_rbac_is_read_only() {
    ...
    // 混合角色时不应该是只读
    assert!(!RbacChecker::is_read_only(&vec!["readonly".to_string(), "user".to_string()]));
}
