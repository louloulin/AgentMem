/*!
MCP (Model Context Protocol) commands for AgentMem CLI

Provides MCP server functionality for integration with Claude Code and other MCP clients.
*/
...
impl McpCommand {
    /// Execute MCP command
    pub async fn execute(subcommand: McpSubcommand) -> Result<()> {
        match subcommand {
            McpSubcommand::Start => {
        ...
        loop {
            ...
            match reader.read_line(&mut line).await {
                ...
                Ok(n) => {
                    ...
                    // 处理请求
                    let response = Self::handle_request(&server, &client, request).await;
