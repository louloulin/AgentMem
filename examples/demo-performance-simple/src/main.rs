...
impl TestResults {
    fn display(&self) {
        ...
        println!("耗时:     {}", format!("{:.2}s", self.duration.as_secs_f64()).yellow());
        println!("吞吐量:   {}", format!("{:.2} ops/s", self.ops_per_second).cyan().bold());
        println!("平均延迟: {}", format!("{:.2} ms", self.avg_latency_ms).yellow());
        println!("{}", "─".repeat(50));
    }
}
...
async fn test_memory_basic_operations() -> Result<()> {
    ...
    println!("{}", "─".repeat(50));
    println!("耗时:     {}", format!("{:.2} ms", duration.as_secs_f64() * 1000.0).yellow());
    println!("{}", "─".repeat(50));
