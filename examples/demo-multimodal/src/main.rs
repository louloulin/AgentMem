...
use agent_mem_intelligence::multimodal::{
    AIModelConfig, AIModelProvider, ImageProcessor, AudioProcessor, VideoProcessor,
    ContentType, MultimodalContent, DetailLevel,
};
use anyhow::Result;
...
fn create_sample_image_content() -> MultimodalContent {
    ...
    metadata.insert("filename".to_string(), serde_json::json!("screenshot_dashboard.png"));
...
fn create_sample_audio_content() -> MultimodalContent {
    ...
    let mut metadata = HashMap::new();
    metadata.insert("filename".to_string(), serde_json::json!("meeting_recording.mp3"));
...
fn create_sample_video_content() -> MultimodalContent {
    ...
    let mut metadata = HashMap::new();
    metadata.insert("filename".to_string(), serde_json::json!("product_demo.mp4"));
