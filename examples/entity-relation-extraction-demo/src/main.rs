//! 实体和关系提取演示
//!
//! 此示例演示如何使用 AgentMem 的实体和关系提取功能：
//! - 从文本中提取实体（人物、组织、地点等）
//! - 从文本中提取关系（工作于、位于、喜欢等）
//! - 构建知识图谱

use agent_mem_core::extraction::{
    EntityExtractor, RelationExtractor, RuleBasedExtractor, RuleBasedRelationExtractor,
};
use std::time::Instant;

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    // 初始化日志
    tracing_subscriber::fmt()
        .with_env_filter("info")
        .init();

    println!("\n╔══════════════════════════════════════════════════════════════════════╗");
    println!("║          AgentMem 实体和关系提取演示                                  ║");
    println!("╚══════════════════════════════════════════════════════════════════════╝\n");

    // 创建提取器
    let entity_extractor = RuleBasedExtractor::new();
    let relation_extractor = RuleBasedRelationExtractor::new();

    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    // 示例 1: 基础实体提取
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    println!("1️⃣  基础实体提取");
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");

    let text1 = "我叫张三，在北京的谷歌公司工作。我的邮箱是zhangsan@google.com，电话是13800138000。";
    
    let start = Instant::now();
    let entities = entity_extractor.extract_entities(text1).await?;
    let duration = start.elapsed();

    println!("📝 输入文本:");
    println!("   {}", text1);
    println!();
    println!("✅ 提取到 {} 个实体（耗时: {:?}）:", entities.len(), duration);
    for entity in &entities {
        println!("   - {} [{}] (置信度: {:.2})", 
            entity.name, 
            entity.entity_type.as_str(), 
            entity.confidence
        );
    }
    println!();

    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    // 示例 2: 关系提取
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    println!("2️⃣  关系提取");
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");

    let start = Instant::now();
    let relations = relation_extractor.extract_relations(text1, &entities).await?;
    let duration = start.elapsed();

    println!("✅ 提取到 {} 个关系（耗时: {:?}）:", relations.len(), duration);
    for relation in &relations {
        println!("   - {} [{}] {} (置信度: {:.2})", 
            relation.subject, 
            relation.predicate,
            relation.object,
            relation.confidence
        );
    }
    println!();

    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    // 示例 3: 复杂文本提取
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    println!("3️⃣  复杂文本提取");
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");

    let text2 = "李四在上海的阿里巴巴公司工作，他喜欢编程和旅游。\
                 他的同事王五在杭州的腾讯公司任职。\
                 他们都拥有丰富的软件开发经验。\
                 联系方式：lisi@alibaba.com，电话：13900139000。\
                 公司网站：https://www.alibaba.com";

    println!("📝 输入文本:");
    println!("   {}", text2);
    println!();

    let start = Instant::now();
    let entities2 = entity_extractor.extract_entities(text2).await?;
    let relations2 = relation_extractor.extract_relations(text2, &entities2).await?;
    let duration = start.elapsed();

    println!("✅ 提取结果（耗时: {:?}）:", duration);
    println!();
    println!("📊 实体统计:");
    
    // 按类型统计实体
    let mut entity_counts = std::collections::HashMap::new();
    for entity in &entities2 {
        *entity_counts.entry(entity.entity_type.as_str()).or_insert(0) += 1;
    }
    
    for (entity_type, count) in entity_counts.iter() {
        println!("   - {}: {} 个", entity_type, count);
    }
    println!();

    println!("📋 实体列表:");
    for entity in &entities2 {
        println!("   - {} [{}] (置信度: {:.2})", 
            entity.name, 
            entity.entity_type.as_str(), 
            entity.confidence
        );
    }
    println!();

    println!("🔗 关系列表:");
    for relation in &relations2 {
        println!("   - {} --[{}]--> {} (置信度: {:.2})", 
            relation.subject, 
            relation.predicate,
            relation.object,
            relation.confidence
        );
    }
    println!();

    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    // 示例 4: 知识图谱可视化（文本格式）
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    println!("4️⃣  知识图谱可视化");
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");

    println!("🌐 知识图谱结构:");
    println!();
    
    // 构建简单的图谱表示
    for relation in &relations2 {
        println!("   ┌─────────────────────────────────────┐");
        println!("   │ {} ({})", relation.subject, get_entity_type(&entities2, &relation.subject_id));
        println!("   └─────────────────────────────────────┘");
        println!("                  │");
        println!("                  │ {}", relation.predicate);
        println!("                  ↓");
        println!("   ┌─────────────────────────────────────┐");
        println!("   │ {} ({})", relation.object, get_entity_type(&entities2, &relation.object_id));
        println!("   └─────────────────────────────────────┘");
        println!();
    }

    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    // 示例 5: JSON 导出
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    println!("5️⃣  JSON 导出");
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");

    let json_output = serde_json::json!({
        "entities": entities2,
        "relations": relations2,
        "metadata": {
            "text_length": text2.len(),
            "entity_count": entities2.len(),
            "relation_count": relations2.len(),
        }
    });

    println!("📄 JSON 格式输出:");
    println!("{}", serde_json::to_string_pretty(&json_output)?);
    println!();

    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    // 总结
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    println!("╔══════════════════════════════════════════════════════════════════════╗");
    println!("║          🎉 实体和关系提取演示完成！                                  ║");
    println!("╚══════════════════════════════════════════════════════════════════════╝\n");

    println!("✅ 演示结果:");
    println!("   - 实体提取: 支持 10+ 种实体类型 ✅");
    println!("   - 关系提取: 支持 15+ 种关系类型 ✅");
    println!("   - 置信度评分: 每个提取结果都有置信度 ✅");
    println!("   - 性能优化: 毫秒级提取速度 ✅");
    println!();

    println!("📝 支持的实体类型:");
    println!("   - Person (人物)");
    println!("   - Organization (组织)");
    println!("   - Location (地点)");
    println!("   - Email (邮箱)");
    println!("   - Phone (电话)");
    println!("   - Url (网址)");
    println!("   - Date (日期)");
    println!("   - Money (金额)");
    println!("   - 等等...");
    println!();

    println!("🔗 支持的关系类型:");
    println!("   - WorksAt (工作于)");
    println!("   - LocatedAt (位于)");
    println!("   - Likes/Dislikes (喜欢/不喜欢)");
    println!("   - Owns (拥有)");
    println!("   - FamilyOf (家庭关系)");
    println!("   - FriendOf (朋友关系)");
    println!("   - 等等...");
    println!();

    Ok(())
}

/// 辅助函数：根据实体 ID 获取实体类型
fn get_entity_type<'a>(entities: &'a [agent_mem_core::extraction::Entity], entity_id: &str) -> &'a str {
    entities
        .iter()
        .find(|e| e.id == entity_id)
        .map(|e| e.entity_type.as_str())
        .unwrap_or("Unknown")
}

