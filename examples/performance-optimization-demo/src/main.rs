...
async fn demo_performance_benchmark() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(
        "  - 平均延迟: {:.2} μs",
        result.vector_search.avg_latency_us
    );
    info!(
        "  - 最小延迟: {:.2} μs",
        result.vector_search.min_latency_us
    );
    info!(
        "  - 最大延迟: {:.2} μs",
        result.vector_search.max_latency_us
    );
    info!(
        "  - P50 延迟: {:.2} μs",
        result.vector_search.p50_latency_us
    );
    info!(
        "  - P95 延迟: {:.2} μs",
        result.vector_search.p95_latency_us
    );
    info!(
        "  - P99 延迟: {:.2} μs",
        result.vector_search.p99_latency_us
    );
    info!(
        "  - 吞吐量: {:.2} ops/s",
        result.vector_search.throughput_ops
    );
    ...
    info!(
        "  - 平均延迟: {:.2} μs",
        result.cache_performance.avg_latency_us
    );
    info!(
        "  - P95 延迟: {:.2} μs",
        result.cache_performance.p95_latency_us
    );
    info!(
        "  - 吞吐量: {:.2} ops/s",
        result.cache_performance.throughput_ops
    );

    info!("\n批量操作性能:");
    info!(
        "  - 平均延迟: {:.2} μs",
        result.batch_operations.avg_latency_us
    );
    info!(
        "  - P95 延迟: {:.2} μs",
        result.batch_operations.p95_latency_us
    );
    info!(
        "  - 吞吐量: {:.2} ops/s\n",
    ...
    // 性能对比
    let cache_speedup =
        result.vector_search.avg_latency_us / result.cache_performance.avg_latency_us;
    info!("性能提升:");
    info!("  - 缓存加速比: {:.2}x", cache_speedup);

    let batch_speedup =
        result.vector_search.avg_latency_us / result.batch_operations.avg_latency_us;
    info!("  - 批量处理加速比: {:.2}x\n", batch_speedup);
...
async fn demo_performance_manager() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(
        "  - 向量相似度阈值: {:.2}",
        vector_params.similarity_threshold
    );
    ...
    info!(
        "  - 批处理成功率: {:.2}%",
        stats.batch_stats.success_rate() * 100.0
    );
    info!(
        "  - 向量优化次数: {}",
        stats.optimizer_stats.vector_optimizations
    );
    info!(
        "  - 图优化次数: {}",
        stats.optimizer_stats.graph_optimizations
    );
